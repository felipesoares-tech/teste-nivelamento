...
public class Fichario {
	...
	private Optional<Veiculo> buscaVeiculoPlaca(String placa) {
		...
		return veiculos.contains(veiculo.get()) ? Optional.ofNullable(veiculos.get(veiculos.indexOf(veiculo.get())))
				: Optional.empty();
	}
	...
	private boolean editarVeiculo(Veiculo veiculo, String valor, int param) {
		...
		case 4:
			...
			return true;
		case 5:
			veiculo.setAnoModelo(Integer.parseInt(valor));
			return true;
		case 6:
			veiculo.setPreco(Float.parseFloat(valor));
			return true;
		default:
			break;
		}
	...
	public void excluirVeiculo() {
		...
		if (veiculo.isPresent()) {
			System.out.print(
					"Tem certeza que deseja excluir o veiculo de placa " + veiculo.get().getPlaca() + " ? [S/n]: ");
	...
	public void alterarVeiculo() {
		...
		if (veiculo.isPresent()) {
			...
			case 1:
				...
				resp = sc.nextLine();
				if (resp.equalsIgnoreCase("S"))
					System.out.println(editarVeiculo(veiculo.get(), novaPlaca, opcao) ? "Placa alterada com sucesso!"
			...
			case 2:
				...
				if (resp.equalsIgnoreCase("S"))
					System.out.println(editarVeiculo(veiculo.get(), marca, opcao) ? "Marca alterada com sucesso"
							: "Erro ao alterar marca do veículo");
				break;
			case 3:
				...
				if (resp.equalsIgnoreCase("S"))
					System.out.println(editarVeiculo(veiculo.get(), modelo, opcao) ? "Modelo alterado com sucesso!"
							: "Errro ao alterar o modelo do veículo !!");
				break;
			case 4:
				...
				if (resp.equalsIgnoreCase("S"))
					System.out.println(editarVeiculo(veiculo.get(),Integer.toString(anoFabricacao), opcao)
							? "Ano fabricação alterado com sucesso !!"
							: "Erro ao alterar o ano de fabricacação !!");
				break;
			...
			case 5:
				...
				if (resp.equalsIgnoreCase("S"))
					System.out.println(
							editarVeiculo(veiculo.get(), Integer.toString(anoModelo), opcao) ? "Ano modelo alterado com sucesso !!"
									: "Erro ao alterar o ano modelo !!");
				break;
			...
			case 6:
				...
				if (resp.equalsIgnoreCase("S"))
					System.out.println(editarVeiculo(veiculo.get(), Float.toString(preco),opcao) ? "Preço alterado com sucesso !!"
							: "Erro ao alterar o preço do veíciulo !!");
				break;
			default:
				break;
			}
